...
Public Class Main_Screen
    ...
    Private Function CleanUpHash(ByVal SourceText As String) As String
        Dim returntext As String = SourceText
        returntext = returntext.Replace("\", "x")
        returntext = returntext.Replace("/", "x")
        returntext = returntext.Replace(":", "x")
        returntext = returntext.Replace("*", "x")
        returntext = returntext.Replace("?", "x")
    ...
    Private Sub PreCount_Function(ByVal worker As BackgroundWorker)
        Try
            ...
            Dim backupfolder As String = (dinfo.FullName & "\").Replace("\\", "\") & "MD5 Backup " & Format(Now, "yyyyMMddHHmmss")
            ...
            For Each finfo As FileInfo In dinfo.GetFiles()
                If (File.GetAttributes(finfo.FullName) And FileAttributes.Hidden) = FileAttributes.Hidden Then
                Else
                ...
                If dinfo.Parent.Exists Then
                    ...
                    For Each finfo As FileInfo In dinfo.GetFiles()
                        ...
                        Try
                            ...
                            Dim savename As String = savedirectory & CleanUpHash(GenerateHash(finfo.Name)) & "_" & finfo.Name
                            Try
                                finfo.CopyTo(savename, False)
                            Catch ex As Exception
                                Error_Handler(ex, "MainWorkerFunctionFileCopy")
                            End Try
                            ...
                            End If
                        Catch ex As Exception
                            Error_Handler(ex, "MainWorkerFunctionFileRename")
                        End Try
                        If worker.CancellationPending Then
                            e.Cancel = True
                            Return False
